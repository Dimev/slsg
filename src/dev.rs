...
pub(crate) fn serve_dev_site(addr: &str) -> anyhow::Result<()> {
    // run the server
    let listener = TcpListener::bind(&addr).context("Failed to serve site")?;

    // time
    let start = Instant::now();

    // generate the initial site
    let mut site = SiteCache::new_site(); //generate(true);
...
fn respond(
    ...
    // otherwise, push the 404 page
        ...
        .as_ref()
        .ok()
        .and_then(|x| x.not_found.as_ref().and_then(|f| x.files.get(f).cloned()))
    {
